...
最后，选择变量的数据类型也是重要的。本例中使用整型 ``int`` 来表示苹果的数量。但是，苹果不可能为负数，我们是否应该使用非负整型 ``unsigned int``\ ？如果苹果允许被切开，我们是否考虑使用带小数的类型，比如双精度浮点型 ``double``\ ？我们将在\ :doc:`DataTypes`\ 一节介绍变量的类型。
...
.. hint::
    ...
    * 如果它除了包含返回类型、函数名、参数，还用花括号包含了函数体，那么它是一个函数定义。
    ...
    在大型代码中，函数经常先在头文件中声明，之后再在源文件中定义。在小型代码中，或者当函数的长度很短时，我们直接定义函数，而不分离声明与定义。
